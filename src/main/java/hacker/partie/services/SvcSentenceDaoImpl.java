...
public class SvcSentenceDaoImpl {
	...
	public static List<SvcSentence> findAll() {
		...
		try {
			...
			SvcSentenceDaoImpl.closeConnections();
		}
    ...
    private static String get_subject(){
        ...
        try {
            myPreparedStatement = connect.prepareStatement("select sentences_svc.subject from sentences_svc order by random() limit 1");
            myResultSet = myPreparedStatement.executeQuery();
            while (myResultSet.next()) {
                randomObject = myResultSet.getString("subject");
            }
